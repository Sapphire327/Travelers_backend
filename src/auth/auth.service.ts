...
export class AuthService {
    ...
    async refresh(refreshToken: string) {
        ...
        const user = await prisma.users.findUnique({ where: { id: tokenFromDb.usersId } })
        if (user) {
            const tokens = this.tokenService.generateTokens({ login: user.login, isAdmin: user.isAdmin })
    ...
    async createAdmin() {
        const findedAdmin = await prisma.users.findUnique({ where: { login: 'mainAdmin' } })
        ...
        if (findedAdmin == null) {
            ...
            await prisma.users.create({ data: { login: login, password: hashedPassword, isAdmin: true } })
        }
    }
}